...
const IdeasListScreen = (props) => {
  ...
  onFilterClose = (categories, sector, sortBy) => {
    ...
    if (categories.toString() != "" || sortBy != 0 || sector.toString() != "") {
      if (tab === undefined || tab === 0) onIdeas("all",categories.toString(), sector.toString(), sortBy);
      else if (tab === 1) onIdeas("latest",categories.toString(), sector.toString(), sortBy);
      else if (tab === 2) onIdeas("popular",categories.toString(), sector.toString(), sortBy);
      else if (tab === 3) onIdeas("winning",categories.toString(), sector.toString(), sortBy);
      else onIdeas("all",categories.toString(), sector.toString(), sortBy);
    }
  };

  const onIdeas = (tabType = "all",categories, sector, sortBy) => {
    const data = {
